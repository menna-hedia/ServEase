...
export default function ServicesPage() {
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    if (requestData.paymentMode === 'HOURLY' && !selectedProvider.hourPrice) {
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2">
                          <Button
                            onClick={() => {
                              setSelectedProvider(provider);
                              setRequestData((prev) => ({ ...prev, paymentMode: 'FIXED', preferredPrice: '' }));
                              setShowRequestModal(true);
                            }}
                          >
                            Request Service
        ...
        <form onSubmit={handleSubmitRequest} className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                disabled={!selectedProvider?.hourPrice}
                onClick={() => setRequestData({ ...requestData, paymentMode: 'HOURLY' })}
                className={`px-4 py-3 rounded-lg border text-sm font-medium transition-colors ${
                  requestData.paymentMode === 'HOURLY'
                    ? 'border-primary bg-primary/10 text-primary'
                    : 'border-border text-muted-foreground hover:bg-muted'
                } ${!selectedProvider?.hourPrice ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Hourly Rate
              </button>
            </div>
            {!selectedProvider?.hourPrice && (
          ...
          {requestData.paymentMode === 'FIXED' ? (
            ...
            <div className="bg-muted rounded-lg p-3">
              <p className="text-sm text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">EGP {selectedProvider?.hourPrice}/hr</span>.
